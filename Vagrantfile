  ...
  yaml.each do |server|
    ...
    config.vm.define server["name"] do |srv|
      ...
      if server["system"] == "ubuntu/bionic64"
        srv.vm.provision "shell", inline: "apt-get update; apt-get install python python-pip python3 python3-pip -y; apt-get update; apt-get install -y software-properties-common; apt-add-repository --yes --update ppa:ansible/ansible;"
      end

      if server["system"] == "centos/7"
        srv.vm.provision "shell", inline:  "yum update; yum install epel-release python2 python2-pip python3 python3-pip policycoreutils-python git wget vim -y;"
      end
